...
// BackgroundOffscreen.instance.sendMessage('workflow:execute');
...
function automaGetFile(path) {
  const id = '${varName}';
   return new Promise((resolve, reject) => {
    const eventName = \`__automa-getFile-response-\${id}__\`;
    const eventListener = ({ detail }) => {
      if (detail.id !== id) return;
    ...
    window.addEventListener(eventName, eventListener);
    window.dispatchEvent(
      new CustomEvent(\`__automa-getFile__\`, {
        detail: {
          id,
          path,
        },
