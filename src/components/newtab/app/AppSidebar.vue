<template>
  <aside class="fixed left-0 top-0 z-50 flex h-screen w-16 flex-col items-center bg-white py-6 dark:bg-gray-800">
    <img :title="`v${extensionVersion}`" src="@/assets/svg/logo.svg" class="mx-auto mb-4 w-10" />
    <div class="relative w-full space-y-2 text-center" @mouseleave="showHoverIndicator = false">
      ...
      <router-link v-for="tab in tabs" v-slot="{ href, navigate, isActive }" :key="tab.id" :to="tab.path" custom>
        <a v-tooltip:right.group="`${t(`common.${tab.id}`, 2)} ${tab.shortcut && `(${tab.shortcut.readable})`
          }`
          " :class="{ 'is-active': isActive }" :href="tab.id === 'log' ? '#' : href"
          class="tab relative z-10 flex w-full items-center justify-center" @click="navigateLink($event, navigate, tab)"
          @mouseenter="hoverHandler">
          <div class="inline-block rounded-lg p-2 transition-colors">
            <v-remixicon :name="tab.icon" />
          </div>
          <span v-if="tab.id === 'log' && runningWorkflowsLen > 0"
    ...
    <button v-tooltip:right.group="$t('home.elementSelector.name')" class="focus:ring-0" @click="injectElementSelector">
...
<div class="w-44">
  <div class="flex items-center">
    ...
    <span title="Subscription" :class="subColors[userStore.user.subscription]"
...
const tabs = computed(() => {
  ...
  return allTabs.filter((tab) => ['workflow', 'storage', 'settings', 'log'].includes(tab.id));
});
...
useShortcut(
  tabs.value.reduce((acc, { shortcut }) => {
    if (shortcut) {
      acc.push(shortcut);
    }
