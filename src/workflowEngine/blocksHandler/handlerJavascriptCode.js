  ...
  if (data.code.includes('automaRefData')) {
    ...
    Object.keys(refData).forEach((keyword) => {
      if (!data.code.includes(keyword)) return;
        ...
        newRefData[keyword] = {};
        Object.keys(refData[keyword] || {}).forEach(dk => {
          if (!data.code.includes(dk)) return;
          else {
            newRefData[keyword][dk] = refData[keyword][dk];
          }
        })
      }
    });
    // payload.refData = { ...newRefData, secrets: {} };
    if (newRefData.secrets){
      newRefData.secrets = Object.keys(newRefData.secrets || {}).reduce((acc, dk) => {
        acc[dk] = credentialUtil.decrypt(newRefData.secrets[dk]);
        return acc;
      }, {});
    };
    payload.refData = { ...newRefData };
  }
