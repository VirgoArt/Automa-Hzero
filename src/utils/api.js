...
export async function fetchClientToken() {
  ...
  const CLIENT_ID_V = await dbStorage.credentials.where('name').equals('HZERO_ID').first();
  const CLIENT_KEY_V = await dbStorage.credentials.where('name').equals('HZERO_KEY').first();

  const CLIENT_ID = credentialUtil.decrypt(CLIENT_ID_V.value);
  const CLIENT_KEY = credentialUtil.decrypt(CLIENT_KEY_V.value);

  const response = await fetch(API_PATH.value + '/oauth/oauth/token?client_id=' + CLIENT_ID + '&client_secret=' + CLIENT_KEY + '&grant_type=client_credentials', {
    method: 'POST',
  });
